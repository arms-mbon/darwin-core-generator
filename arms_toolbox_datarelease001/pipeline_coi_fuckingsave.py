...
class PipelineCOI(Pipeline):
    ...
    def update_cache(self, occurrence_id, taxon_ncbi_tax_id):
        if not taxon_ncbi_tax_id in self.cache.keys():
            ...
            self.cache[taxon_ncbi_tax_id]["scientificName"] = self.ncbi2aphia[ncbi_tax_id].get("scientificname", "scientificname") # or use scientificName = self.cache[taxon_ncbi_tax_id]["scientificName"] as second one?
            self.cache[taxon_ncbi_tax_id]["taxonRank"] = self.ncbi2aphia[ncbi_tax_id].get("rank", "rank")
