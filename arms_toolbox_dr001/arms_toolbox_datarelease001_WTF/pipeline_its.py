...
TO ADD
when get the classification and sciname, match the two and then get the name-before-the-scienam from the class
then when find no match to aphia, look at the rank (from ena) and if species, then take the  
name-b4-sciname and push that one thru the worms check. output i a file that have done that
...
class PipelineITS(Pipeline):
    ...
    def run(self):
        ...
        if self.genomic_region == "ITS":
            ...
            full_file_name = f"https://raw.githubusercontent.com/arms-mbon/analysis_release_002/main/taxonomic_assignments/Extended_final_table_{self.time_window}_{self.genomic_region}_noBlank.csv"
            ...
            # for data release 002  
            full_file_name = f"https://raw.githubusercontent.com/arms-mbon/analysis_release_002/main/taxonomic_assignments/Extended_final_table_{self.time_window}_{self.genomic_region}_noBlank_TaxonomyCurated.csv"
            print(f"{full_file_name=}\n{self.time_window=}\n{self.genomic_region}")
            df_pema = pd.read_csv(full_file_name)
        else:
            full_file_name = "None"
